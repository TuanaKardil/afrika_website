"""Spider for UNDP procurement notices."""
import re
...
_BASE = "https://procurement-notices.undp.org"
_START = _BASE + "/"
...
class UndpTendersSpider(BaseTenderSpider):
    name = "undp_tenders"
    allowed_domains = ["procurement-notices.undp.org"]
    start_urls = [_START]
    source_key = "undp"

    def parse(self, response: Response):
        # UNDP procurement listing table
        for row in response.css(
            "table.table tbody tr, div.procurement-item, tr.notice-row"
        ):
            ...
            deadline_text = row.css(
                "td.deadline::text, td:nth-child(4)::text, .closing::text"
            ).get()
            deadline_at = _parse_date(deadline_text)

            published_text = row.css(
                "td.published::text, td:nth-child(3)::text, .pub-date::text"
            ).get()
        ...
        next_page = response.css(
            "a[rel='next']::attr(href), li.next a::attr(href), a.next-page::attr(href)"
        ).get()
        if next_page:
    ...
    def parse_tender(self, response: Response):
        title = (
            response.css("h1.notice-title::text, h1.title::text, h1::text").get()
        ...
        description = (
            " ".join(
                response.css(
                    "div.notice-body *::text, div.description *::text, div.content *::text"
                ).getall()
            ).strip()
            or ""
        )
        ...
        reference_number = (
            response.css(
                ".reference::text, .notice-ref::text, .contract-id::text"
            ).get() or ""
        ).strip()

        institution = (
            response.css(".organization::text, .agency::text, .department::text").get()
            or "UNDP"
        ).strip()

        country = (
            response.css(".country::text, .location::text, td.country::text").get() or ""
        ).strip()

        raw_published = response.css(
            "time::attr(datetime), .publication-date::text, .posted-date::text"
        ).get()
        published_at = _parse_date(raw_published) or response.meta.get("published_at")

        raw_deadline = response.css(
            ".deadline::text, .closing-date::text, .submission-deadline::text"
        ).get()
        deadline_at = _parse_date(raw_deadline) or response.meta.get("deadline_at")

        budget_text = (
            response.css(".budget::text, .estimated-value::text, .contract-value::text").get() or ""
        ).strip()
