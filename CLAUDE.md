...
22. **A slug is permanent once assigned; never recompute it on update.** `StoragePipeline` used to include `slug` in the `is_update` write. `_make_slug()` appends a random 6-hex suffix only on collision, and on re-scrape an article always collides with its own stored slug, so every content update flipped the public URL (`base` → `base-a1b2c3` → `base` → …) and 404-ed the previously indexed/shared link. 120 of 840 rows carried a hex suffix while **zero** bases were shared by two articles, proving the suffixes came from self-collision. `slug` now sits alongside `title_tr`/`excerpt_tr`/`content_tr` in the update-exclusion set — keep it there, and never add a slug rewrite to any backfill or admin PATCH. Already-broken legacy URLs are rescued at read time: `resolveLegacyArticleSlug()` (`lib/queries/articles.ts`) strips the hex suffix and matches `slug.eq.<base>` OR `slug.like.<base>-______` (exactly six wildcard chars, so a longer unrelated slug sharing the prefix cannot match), and `app/haber/[slug]` `permanentRedirect`s (308) to the current slug instead of `notFound()`. Unknown slugs still 404.