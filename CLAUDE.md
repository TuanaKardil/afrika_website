...
12. **Deployments go from the repo root:** Run `vercel deploy --prod --yes` from the repo root (`~/dev/afrika website/`, moved out of iCloud-synced `~/Desktop` in July 2026), not from `frontend/`. The `frontend` Vercel project is a ghost — `www.afrikahaberleri.tr` is served by the `afrika-website` project linked to the repo root.
