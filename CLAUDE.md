...
| **Clickable links in articles** | `sanitize.ts` was stripping `<a>` (and `<small>`, the `class` on `.source-link`), so source citations and internal links rendered as dead plain text. Added `a` + `small` to ALLOWED_TAGS and `a: [href, target, rel]` + `p: [class]` to allowedAttributes. sanitize-html still blocks unsafe href schemes (`javascript:`) and strips `onclick`. Followed source links + internal links are a top SEO/GEO signal — keep `a` allowed. |
